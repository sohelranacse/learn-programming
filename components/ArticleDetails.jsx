    ...
    <section className="px-2 mt-14 min-h-screen">
      <div className="container mx-auto md:grid grid-cols-12 h-full">
        <div className="col-span-9 h-full py-6 md:px-6">
          <h1 className="text-4xl pb-3 dark:text-gray-200">Post Titles</h1>
          <div className="dark:text-gray-200">Post details here</div>
          <MyCode
            code={`function appoint_waiting_candidate($recruitment_id, $candidate_id) {
	$result = $this->recruitment_mdl->appoint_waiting_candidate($recruitment_id, $candidate_id);
	if($result)
		$data['success'] = "Appointed Successfully!";
	else			
		$data['error'] = "";
